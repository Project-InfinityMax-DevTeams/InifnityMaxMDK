...
public final class ExampleModRegistrations {
     ...
     *
     * このクラスはユーティリティ的に静的メソッドのみを提供するため、外部からの生成を防止する目的で定義されている。
     */
    private ExampleModRegistrations() {
    }

    /**
     * MOD の代表的なゲーム要素（ブロック、エンティティ、ブロックエンティティ、クライアント初期化、GUI、ディメンション、
     * バイオーム、アイテム、パケット、データ生成、カスタム拡張など）をサンプル登録した登録DSLを作成する。
     *
     * <p>作成された DSL はサンプル設定（名前、実装クラスパス、各種プロパティ）で事前に填充されている。</p>
     *
     * @return 初期設定済みの ModRegistrationDsl インスタンス
     */
